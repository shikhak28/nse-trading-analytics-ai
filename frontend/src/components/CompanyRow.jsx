const columns = "grid grid-cols-[1.8fr_0.9fr_0.8fr_0.9fr_1.1fr_1.1fr_0.9fr_0.9fr_0.9fr] items-center gap-3";
...
export function CompanyRow({ index, style, companies, quotes = {} }) {
  const company = companies[index];
  const quote = quotes[company.symbol];

  const ltp = quote?.ltp ?? company.last_close;
  const changePercent = quote?.change_percent;
  const volume = quote?.volume ?? company.last_volume;
  ...
  const buyQty = quote?.total_buy_quantity;
  const sellQty = quote?.total_sell_quantity;

  return (
    <div
      style={style}
      className={`${columns} px-3 border-b border-slate-100 dark:border-slate-800 hover:bg-slate-100/70 dark:hover:bg-slate-800/70 transition text-[12px]`}
    >
